...
const STYLE_INSTRUCTIONS: Record<SummaryStyle, string> = {
  ...
  detailed:
    'Write a detailed summary mainly as flowing prose (2–4 short paragraphs). You may add at most 3–5 bullet points for discrete highlights only — do not turn the whole summary into a bullet list, and do not add section headings like "Key points".',
};
...
export function buildSummarySystemPrompt(
  ...
  const declaredLanguage = pageLanguage?.trim().toUpperCase();
  const languageInstruction = outputLanguage
    ...
    : `Write the entire summary in the same primary language as the source text.${
        declaredLanguage
          ? ` The page declares its language as "${declaredLanguage}". Treat that as a strong hint, but follow the language actually used by the source if they conflict.`
          : ''
  ...
  return [
    ...
    `${STYLE_INSTRUCTIONS[style]} After the summary, leave a blank line, then end with exactly one final line in this form: **Takeaway:** <one sentence>. Never put the Takeaway inside a bullet list.`,
  ].join(' ');
}
...
export function buildPageContent(input: {
  ...
  const language = input.language?.trim()
    ? `\nPAGE LANGUAGE: ${input.language.trim().toUpperCase()}`
    : '';
