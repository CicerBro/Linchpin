...
export class AnthropicProvider implements SummarizerProvider {
  ...
  async summarize(request: SummarizeRequest): Promise<string> {
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) throw new ProviderError('Anthropic rejected the API key.', 'auth', response.status);
      if (response.status === 429) throw new ProviderError('Anthropic rate limit was reached. Try again later.', 'rate-limit', 429);
      if (!response.ok) throw new ProviderError(`Anthropic request failed (HTTP ${response.status}).`, 'response', response.status);
      const body: unknown = await response.json();
      const blocks = (body as { content?: Array<{ type?: string; text?: unknown }> }).content;
      const text = blocks?.filter((item) => item.type === 'text' && typeof item.text === 'string').map((item) => item.text).join('\n');
      if (!text?.trim()) throw new ProviderError('Anthropic returned a malformed response.', 'response');
      return text.trim();
    } catch (error) {
